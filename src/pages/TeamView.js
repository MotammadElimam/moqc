...
class TeamView extends React.Component {
    ...
    getStudentProfile = async () => {
        ...
        const response = await axios.post(`https://moqc.ae/api/profile/${team_id}`, '',
            {
                headers: { "token": token }
            });
    ...
    update = async () => {
        ...
        const response = await axios.post(`https://moqc.ae/api/editProfile/${team_id}`,
            body,
            {
                headers: { "token": token }
            });
    ...
    async docPicker() {
        try {
            ...
            axios.post(`https://moqc.ae/api/profile_upload/${team_id}`, body,
    ...
    async uploadAPICall() {
        ...
        axios.post(`https://moqc.ae/api/profile_upload`, body)
    ...
    async downloadFile(link) {
        ...
        config(options)
            .fetch('GET', `https://moqc.ae/${this.state.passportimage}`)
            .then((res) => {
                                ...
                                <View style={{ backgroundColor: '#ffff', borderRadius: 10, borderWidth: 1, borderColor: '#D5D5D5', padding: 10 }}>
                                    <View style={{ backgroundColor: '#F7F8FA', borderRadius: 10, padding: 10 }}>
                                        <View style={{ marginVertical: 5, flexDirection: 'column', alignItems: "center" }}>
                                            <Text style={{marginVertical:5, width: '100%', fontWeight: 'bold' }}>{i18n.t('UserName')} </Text>
                                        ...
                                        <View style={{ flexDirection: 'row' }}>
                                            <View style={{ width: '30%', marginVertical: 5, flexDirection: 'column', alignItems: "center" }}>
                                                ...
                                                <View style={{ width: '100%', flexDirection: 'row', paddingTop: 5, height: 50, borderWidth: 0, borderRadius: 10 }}>
                                                    {/* <Pressable ><Text>{this.state.studentDetail.passport}</Text></Pressable> */}
                                                    <Icon onPress={() => { this.docPicker(); this.setState({ attachmentname: 'passport' }) }} active size={20} name='attach-file' type="MaterialIcons" style={{ color: "#579976", fontSize: 20, marginHorizontal: 1 }} />
                                                    <Icon onPress={() => this.historyDownload()} active size={20} name='file-download' type="MaterialIcons" style={{ color: "#579976", fontSize: 20, marginHorizontal: 1 }} />
                                                </View>
                                            </View>
                                            <View style={{ width: '70%', marginVertical: 5, flexDirection: 'column', alignItems: "center" }}>
                                                ...
                                                <TextInput style={{ width: '100%', padding: 10, height: 40, borderWidth: 1, borderRadius: 10 }}
                                                    onChangeText={(e) => {
                                                        var clas = this.state.studentDetail
                                                        clas.passport_expiration = e
                                                        this.setState({ studentDetail: clas })
                                                    }}>{item.passport_expiration}</TextInput>
                                            </View>
                                        ...
                                        <View style={{ flexDirection: 'row' }}>
                                            <View style={{ width: '30%', marginVertical: 5, flexDirection: 'column', alignItems: "center" }}>
                                                ...
                                                <View style={{ width: '100%', flexDirection: 'row', padding: 5, height: 40, borderWidth: 0, borderRadius: 10 }}>
                                                    {/* <Pressable>
                                                    <Text>{this.state.studentDetail.emirates_id}</Text></Pressable> */}
                                                    <Icon onPress={() => { this.docPicker(); this.setState({ attachmentname: 'emirates_id' }) }} active size={20} name='attach-file' type="MaterialIcons" style={{ color: "#579976", fontSize: 20, marginHorizontal: 1 }} />
                                            ...
                                            <View style={{ width: '70%', marginVertical: 5, flexDirection: 'column', alignItems: "center" }}>
                                                ...
                                                <TextInput style={{ width: '100%', padding: 10, height: 40, borderWidth: 1, borderRadius: 10 }}
                                                    onChangeText={(e) => {
                                                        var clas = this.state.studentDetail
                                                        clas.emirates_expiration = e
                                                        this.setState({ studentDetail: clas })
                                                    }}>{item.emirates_expiration}</TextInput>
                                            </View>
...
export default withTranslation()(TeamView)
