...
class ResetAdminPassword extends React.Component {
    ...
    reset = async () => {
        ...
        if (response.status === 200) {
            ...
            this.setState({password:'',confpass:''})
        }
    }
                ...
                <ImageBackground
                    ...
                    <View style={{ borderWidth: 1, borderRadius: 20, borderColor: '#D5D5D5', padding: 20 }}>
                        ...
                        <View style={{ marginVertical: 5, flexDirection: 'column', alignItems: "center" }}>
                            <Text style={{ width: '100%', fontWeight: 'bold' }}>{i18n.t('New Password')}</Text>
                            <TextInput style={{marginVertical:5, padding: 10, height: 40, width: '100%', borderWidth: 1, borderRadius: 10 }}
                                onChangeText={(e) => {
                        ...
                        <View style={{ marginVertical: 5, flexDirection: 'column', alignItems: "center" }}>
                            <Text style={{ width: '100%', fontWeight: 'bold' }}>{i18n.t('Confirm Password')}</Text>
                            <TextInput style={{marginVertical:5, padding: 10, height: 40, width: '100%', borderWidth: 1, borderRadius: 10 }}
                                onChangeText={(e) => {
                        ...
                        <Pressable disabled={!this.state.password || !this.state.confpass} onPress={() => this.reset()} style={{ bottom: 0, alignSelf: 'flex-end' }}><Text style={{ fontWeight: 'bold', fontSize: 16, color: 'green' }}>{i18n.t('Reset')}</Text></Pressable>
                    </View>
                </ImageBackground>
...
export default withTranslation()(ResetAdminPassword)
